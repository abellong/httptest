...
Timer*
tmr_create(
    struct timeval* nowP, TimerProc* timer_proc, long msecs, int periodic )
    {
    Timer* t;
    ...
    t->time.tv_sec += msecs / 1000000L;
    t->time.tv_usec += msecs % 1000000L;
...
tmr_timeout( struct timeval* nowP )
    ...
    timeout.tv_sec = msecs / 1000000L;
    timeout.tv_usec = msecs % 1000000L;
    return &timeout;
    }
...
tmr_run( struct timeval* nowP )
	...
	for ( t = timers[h]; t != (Timer*) 0; t = next )
	    ...
	    */
		...
		{
		/* Reschedule. */
		t->time.tv_sec += t->msecs / 1000000L;
		t->time.tv_usec += t->msecs % 1000000L;
		if ( t->time.tv_usec >= 1000000L )
		    {
		    t->time.tv_sec += t->time.tv_usec / 1000000L;
		    t->time.tv_usec %= 1000000L;
